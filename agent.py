...
class Agent:
        ...
        def update_q_table(self, environment, action, reward):
            """
            Updates the Q-table based on the observed transition.
            :param environment: Environment object
            :param action: str representing the taken action
            :param reward: float representing the received reward
            """
            state = environment.get_state(self)
            next_state = environment.get_state(self)
            
            if state not in self.q_table:
                self.q_table[state] = {a: 0.0 for a in environment.get_available_actions(self)}
            if next_state not in self.q_table:
                self.q_table[next_state] = {a: 0.0 for a in environment.get_available_actions(self)}
